...
(function () {
  ...
  const styles = `
      ...
      .mart-action-btn.whatsapp:hover {
  ...
  const addMessage = (role, text, actions = []) => {
    ...
    // Ajouter bouton WhatsApp si demandé
    if (actions && actions.length > 0) {
      actions.forEach(action => {
        if (action.type === 'whatsapp_link') {
          const btnContainer = document.createElement('div');
          btnContainer.style.marginTop = '8px';

          const btn = document.createElement('a');
          btn.href = action.url || "https://wa.me/33650754389";
          btn.target = '_blank';
          btn.rel = 'noopener';
          btn.className = 'mart-action-btn whatsapp';
          btn.innerHTML = `
